...
// Javascript way
function dhtmlLoadScript(url,type,rel,charset,inbase64){
...
function dhtmlLoadScript_submit(url,frm,type,rel,charset,inbase64){
	var e = document.createElement("script");
	var parameters;
	if(inbase64){
...
function ajax_submit(frm,target,afterJSFunction,inbase64){
...
function ajax_submitExt(frm,action,target,afterJSFunction,inbase64){
...
function ajax_submit_json(frm,target,afterJSFunction,inbase64){
	...
	if(document.getElementById(target) ){
		...
		ajax_makeJSONRequest(frm.action,json,target,afterJSFunction);
	}else{	
		frm.submit();
	}		
}

function ajax_submitExt_json(frm,action,target,afterJSFunction,inbase64){
	...
	if(document.getElementById(target) ){
		...
		ajax_makeJSONRequest(action,json,target,afterJSFunction);
	}else{	
		frm.submit();
	}		
}
...
function ajax_submit_mpart(frm,target,afterJSFunction,inbase64){
	if(window.FormData){
		...
		if(document.getElementById(target) ){
			...
			ajax_makeMPARTRequest(frm.action,formdata,target,afterJSFunction);
		}else{	
			frm.submit();
		}	
	}else{
...
function ajax_submitExt_mpart(frm,action,target,afterJSFunction,inbase64){
	if(window.FormData){
		if(document.getElementById(target) ){
			var formdata;	
			if(inbase64)
				formdata = ajax_makeMPARTParameters64(frm,url);
			else formdata = ajax_makeMPARTarameters(frm,frm.action);
			ajax_makeMPARTRequest(action,formdata,target,afterJSFunction);
		}else{	
			frm.submit();
		}
	}else{
