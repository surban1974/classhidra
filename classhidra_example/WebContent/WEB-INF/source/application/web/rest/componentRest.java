...
public class componentRest extends action implements i_action, Serializable{
...
@ActionCall(
	...
	Redirect=@Redirect(contentType="application/json")
			...
			@Parameter(name="items") List list){
		for(int i=0;i<list.size();i++){
			SimpleItem current = (SimpleItem)list.get(i);
			if(current.getId()==id)
				return
...
@ActionCall(
	...
	Redirect=@Redirect(contentType="application/json")
)	public response_wrapper postItem(@Parameter(name="item") SimpleItem item){
		if(item==null)
			return 
					new response_wrapper()
					.setContent("{\"error\":\"Item entity is missing\"}")
