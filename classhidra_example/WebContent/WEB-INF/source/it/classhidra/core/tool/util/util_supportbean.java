...
//******************************
				...
				if(key.indexOf(".")==-1){
					...
					if(current_requested!=null){
						try{
							...
							try{
								if(format!=null)
									setCampoValuePoint(current_requested,key,util_makeValue.makeFormatedValue(bean,format,value,getCampoValue(bean,key),replaceOnBlank,replaceOnErrorFormat));
								else setCampoValuePoint(current_requested,key,util_makeValue.makeValue(value,getCampoValue(bean,key)));
							}catch(Exception ex){
							}
						}
					}
	...
	public static void setCampoValuePoint(Object req, String nome, Object value) throws Exception{
