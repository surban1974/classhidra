	...
	for(int i=0;i<dati.size();i++){
		...
		if(cr.trim().equals(""))
			...
			try{
				...
				if(forAdd==null) forAdd = util_reflect.prepareWriteValueForTag(current,"get",cr,null);
...
//------------------------------
public static List _clone_sort(List dati, List dati_cr) {
	...
	for(int i=0;i<index.size();i++)
		result.add(dati.get(((Integer)index.get(i)).intValue()));
	return result;
}



public static List sort(List dati, String[] cr_array) {
	String[] cr_ad = new String[cr_array.length];
	for(int i=0;i<cr_ad.length;i++) cr_ad[i]="A";
	return sort(dati,cr_array,cr_ad);
}
public static List sort(List dati, String[] cr_array, String[] cr_ad) {
	if(dati==null || dati.size()<=1) return dati;
	dati = sort(dati,cr_array[0],cr_ad[0]);
...
public static List sort(List dati, String cr) {
	return sort(dati,cr,"A");
}
public static List sort(List dati, String cr, String ord) {
	List dati_cr = new ArrayList();
	for(int i=0;i<dati.size();i++){
		Object current = dati.get(i);
		if(cr.trim().equals(""))
			dati_cr.add(current);
		else{
			try{
...
//----------------------------
private static Object getObject(Object current, String method) {
	try{
...
private static void _clone_sortSelf(List dati, List dati_cr, int start, int finish) {
	if(	dati==null ||
		dati.size()==0 ||
		start<0 ||
		start>dati.size() ||
...
//	List result = new ArrayList();
//	try{
//		result = (List)util_cloner.clone(dati);
//	}catch(Exception e){
//	}
	List index = new ArrayList();
	for(int i=start;i<finish;i++)
