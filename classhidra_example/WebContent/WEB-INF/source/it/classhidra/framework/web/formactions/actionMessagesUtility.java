...
public class actionMessagesUtility extends action implements i_action, Serializable{
	private static final long serialVersionUID = -3476492198668495035L;
	String integrity_log;
	HashMap presented ;
...
//		String pathInput=(rootPath+((String)get_bean().get("pathInput"))).replace("//","/");
//		String mask_name=((String)get_bean().get("pathInput")).replace("//","/").replace('/','.');

		String inp = (String)get_bean().get("pathInput");
		inp = util_format.replace(inp,"\\","/");
		String pathInput= "";
		if( !inp.equals("/") && new File(inp).exists()){
			pathInput=util_format.replace( (((String)get_bean().get("pathInput"))),"//","/");
		}else{
			pathInput=util_format.replace( (rootPath+((String)get_bean().get("pathInput"))),"//","/");
		}
