...
public class JsonWriter2 {
	...
	private static String generateJsonItem(Object sub_obj, String name, int level, boolean notFirst, Map<Integer,String> avoidCyclicPointers,
			Serialized annotation, boolean serializeChildren, int serializeDepth, Map<String,?> treeFilters, String subFilterName, WriteValidator validator,
		...
		if(treeFilters!=null){
			if(name!=null && treeFilters.get(name)==null && treeFilters.get(util_reflect.revAdaptMethodName(name))==null)
				...
				@SuppressWarnings("unchecked")
				Map<String,?> map = (Map<String,?>)treeFilters.get(subFilterName);
				subTreeFilters = map;
				if(subTreeFilters==null){
					@SuppressWarnings("unchecked")
					Map<String,?> map2 = (Map<String,?>)treeFilters.get(util_reflect.revAdaptMethodName(subFilterName));
					subTreeFilters = map2;
				}
			}
		}
	...
	private static String generateJsonItemOnlyContent(Object sub_obj, String name, int level, boolean notFirst, 
			...
			Map<String,?> treeFilters, String subFilterName, WriteValidator validator, Stack<Object> objList){
		String result="";
		boolean goAhead = true;
		Map<String,?> subTreeFilters = null;
		if(treeFilters!=null){
