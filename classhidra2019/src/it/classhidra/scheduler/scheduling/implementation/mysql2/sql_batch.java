...
public class sql_batch {
	...
	public static String sql_LoadBatch(HashMap<String,?> form){
		...
		result+="LEFT JOIN  \n";
		result+="( select max(tm_fin) as tm_fin ,cd_btch, cd_ist  from "+b_init.get_db_prefix()+"batch_log \n";
		result+="group by cd_btch, cd_ist ) L \n";
		result+="ON  B.cd_btch = L.cd_btch \n";
		result+="and  B.cd_ist = L.cd_ist \n";
	...
	public static String sql_LoadBatchLog(HashMap<String,?> form){
		...
		String result="";
		result+="select cd_ist,cd_btch,tm_start,tm_fin,st_exec, dsc_exec from "+b_init.get_db_prefix()+"batch_log \n";
		result+="WHERE \n";

		result+="cd_btch = '"+form.get("cd_btch")+"'  \n";
		if(form.get("cd_ist")!=null)
			result+="and cd_ist = "+form.get("cd_ist")+"  \n";
